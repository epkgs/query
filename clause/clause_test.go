...
func TestNeqExpression(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			neq:          Neq{Col: "name", Val: "test"},
			expected:     "`name` <> $1",
			expectedVars: []interface{}{"test"},
		},
		{
			name:         "Neq with numeric value",
			neq:          Neq{Col: "age", Val: 18},
			expected:     "`age` <> $1",
			expectedVars: []interface{}{18},
		},
		{
			name:         "Neq with nil value",
			neq:          Neq{Col: "email", Val: nil},
			expected:     "`email` IS NOT NULL",
			expectedVars: []interface{}{},
		},
		{
			name:         "Neq with slice value",
			neq:          Neq{Col: "id", Val: []interface{}{1, 2, 3}},
			expected:     "`id` NOT IN ($1,$2,$3)",
			expectedVars: []interface{}{1, 2, 3},
		},
	}
...
func TestComparisonExpressions(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "Gt expression",
			expr:         Gt{Col: "age", Val: 18},
			expected:     "`age` > $1",
			expectedVars: []interface{}{18},
		},
		{
			name:         "Gte expression",
			expr:         Gte{Col: "age", Val: 18},
			expected:     "`age` >= $1",
			expectedVars: []interface{}{18},
		},
		{
			name:         "Lt expression",
			expr:         Lt{Col: "age", Val: 30},
			expected:     "`age` < $1",
			expectedVars: []interface{}{30},
		},
		{
			name:         "Lte expression",
			expr:         Lte{Col: "age", Val: 30},
			expected:     "`age` <= $1",
			expectedVars: []interface{}{30},
		},
		{
			name:         "Like expression",
			expr:         Like{Col: "name", Val: "%test%"},
...
func TestAndExpression(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expr:         And(Eq{Col: "name", Val: "test"}, Gt{Col: "age", Val: 18}),
			expected:     "(`name` = $1 AND `age` > $2)",
			expectedVars: []interface{}{"test", 18},
		},
		{
			name:         "And with single condition",
			expr:         And(Eq{Col: "name", Val: "test"}),
			expected:     "`name` = $1",
			expectedVars: []interface{}{"test"},
		},
		{
			name:         "And with nested Or",
			expr:         And(Eq{Col: "status", Val: "active"}, Or(Eq{Col: "age", Val: 18}, Eq{Col: "age", Val: 19})),
...
func TestOrExpression(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expr:         Or(Eq{Col: "name", Val: "test"}, Eq{Col: "name", Val: "admin"}),
			expected:     "(`name` = $1 OR `name` = $2)",
			expectedVars: []interface{}{"test", "admin"},
		},
		{
			name:         "Or with single condition",
			expr:         Or(Eq{Col: "name", Val: "test"}),
			expected:     "`name` = $1",
			expectedVars: []interface{}{"test"},
		},
		{
			name:         "Or with nested And",
			expr:         Or(And(Eq{Col: "name", Val: "test"}, Gt{Col: "age", Val: 18}), And(Eq{Col: "name", Val: "admin"}, Gt{Col: "age", Val: 25})),
...
func TestNotExpression(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "Not with Eq",
			expr:         Not(Eq{Col: "name", Val: "test"}),
			expected:     "`name` <> $1",
			expectedVars: []interface{}{"test"},
		},
		{
			name:         "Not with IN",
			expr:         Not(IN{Col: "id", Vals: []interface{}{1, 2, 3}}),
			expected:     "`id` NOT IN ($1,$2,$3)",
			expectedVars: []interface{}{1, 2, 3},
		},
		{
			name:         "Not with And",
			expr:         Not(And(Eq{Col: "name", Val: "test"}, Gt{Col: "age", Val: 18})),
			expected:     "(`name` <> $1 AND `age` <= $2)",
			expectedVars: []interface{}{"test", 18},
		},
		{
			name:         "Not with Or",
			expr:         Not(Or(Eq{Col: "name", Val: "test"}, Eq{Col: "name", Val: "admin"})),
